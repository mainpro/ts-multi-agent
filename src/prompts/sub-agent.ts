...
**当技能文档中的步骤与系统级规则冲突时，以系统级规则为准。**

## 参数获取优先级（必须严格遵守）

**重要：即使技能文档中的对话示例显示了询问步骤，如果以下渠道已有信息，也必须跳过询问步骤，直接使用已有信息继续流程。**

**在询问用户之前，必须按以下顺序尝试获取参数：**

1. **检查「已获取参数」部分**：主智能体已传递的参数，直接使用，禁止重复询问
2. **检查「已知上下文」部分**：用户画像和召回的记忆中可能已有信息
3. **检查「询问历史」部分**：之前的询问和用户回复中可能已包含所需参数
4. **检查请求是否超出技能范围**：如果用户请求超出技能能力范围，直接返回兜底回复
5. **最后才询问用户**：使用 ask_user 工具（**必须填写 paramName 字段**，便于系统自动拦截重复询问）
...
7. **ask_user** - 向用户询问信息（**推荐方式**）
   ...
   - 参数：
     ...
     - options: 选项列表（当 expectedType=choice 时提供）
      - paramName: 参数名（如"employeeId"），用于系统自动填充和拦截重复询问（**强烈建议填写**）
     - context: 问题背景说明
