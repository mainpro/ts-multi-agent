...
async function run() {
  ...
  // ========================================================================
  // MA-01: recall_prompt 结果
  // ========================================================================
  console.log('--- processRequirement 入口测试 ---');

  await test('MA-01: recall_prompt -> 返回 success, type=recall_prompt', async () => {
    // MainAgent 内部创建了自己的 SessionStore，所以我们需要通过
    // processRequirement 的正常流程来触发 recall_prompt。
    // 策略：先发一个请求（会被处理为 new_request），然后挂起它，
    // 再发一个相关的新请求触发 recall_prompt。
    // 但 MainAgent 没有暴露挂起请求的公共方法。
    //
    // 另一种策略：直接利用 RequestManager 的行为。
    // RequestManager.handleUserInput 会检查 suspended 请求。
    // 由于 MainAgent 内部创建 SessionStore，我们需要让同一个 SessionStore
    // 先有一个 suspended 请求。
    //
    // 方案：第一次调用 processRequirement 会创建一个请求。
    // 但请求完成后状态变为 completed，不是 suspended。
    // 我们需要让请求进入 suspended 状态。
    //
    // 最简单的方式：直接在磁盘上创建 session.json 文件，
    // 包含一个 suspended 请求。然后 MainAgent 的 SessionStore 会读取它。

    const userId = `user-ma01-${Date.now()}`;
    const sessionId = userId;

    // 直接在磁盘上创建 session.json
    const sessionDir = `data/memory/${userId}/${sessionId}`;
    await fs.mkdir(sessionDir, { recursive: true });

    const suspendedReq = makeRequest({
      requestId: 'req-suspended-001',
      content: '旧请求内容',
      status: 'suspended',
      suspendedAt: new Date().toISOString(),
      suspendedReason: '测试挂起',
    });
    ...
    // Mock LLM: shouldRecall 返回 true
    const mockLLM = createMockLLM({
      generateText: async (prompt: string) => {
        if (prompt.includes('挂起的请求')) {
          return '{"shouldRecall": true, "confidence": 0.9, "reason": "相关"}';
        }
        return '{}';
      },
    });
    ...
    const result = await agent.processRequirement('相关的新请求', undefined, userId, sessionId);

    assert.strictEqual(result.success, true, '应该返回 success=true');
    assert.strictEqual((result.data as any)?.type, 'recall_prompt', '应该返回 type=recall_prompt');
    assert.ok((result.data as any)?.suspendedRequest, '应该包含 suspendedRequest');
    assert.strictEqual(
      (result.data as any).suspendedRequest.content,
      '旧请求内容',
      'suspendedRequest 内容应该匹配'
    );
  });
  ...
  await test('MA-07: recall 不存在的请求 -> 返回 success=false, code=REQUEST_NOT_FOUND', async () => {
    const mockLLM = createMockLLM();
    const taskQueue = new MockTaskQueue();
    const agent = new MainAgent(mockLLM, mockSkillRegistry, taskQueue, 3);

    const result = await agent.recallRequest('user-notexist', 'session-notexist', 'req-notexist');

    assert.strictEqual(result.success, false, '应该返回 success=false');
    assert.strictEqual(result.error?.code, 'REQUEST_NOT_FOUND', '应该返回 code=REQUEST_NOT_FOUND');
  });

  // ========================================================================
  // MA-08: recall 存在的请求
  // ========================================================================
  await test('MA-08: recall 存在的请求 -> 调用 processNormalRequirement 不抛异常', async () => {
    const userId = `user-ma08-${Date.now()}`;
    const sessionId = userId;

    // 在磁盘上创建包含 suspended 请求的 session
    const sessionDir = `data/memory/${userId}/${sessionId}`;
    await fs.mkdir(sessionDir, { recursive: true });
    ...
    // recallRequest 会恢复请求并调用 processNormalRequirement
    // "需要召回的请求" 不匹配 small_talk 模式，会走 LLM 路径
    // mock LLM 返回 {}，IntentRouter 的 llmMatchSkillWithSignals 会解析失败
    // classify 的 catch 返回 unclear，handleNonSkillIntent 返回 success=true, type=unclear
    const result = await agent.recallRequest(userId, sessionId, req.requestId);
