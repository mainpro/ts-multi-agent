...
describe('POST /tasks/stream 503 queue-full path', () => {
  let dataDir: string;
  let server: http.Server;
  let url: string;
  let sessionStore: SessionStore;
  ...
  beforeEach(async () => {
    dataDir = path.join(os.tmpdir(), `apiqf-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`);
    await fs.mkdir(path.join(dataDir, 'memory'), { recursive: true });
  ...
  test('returns 503 + QUEUE_FULL when pending queue reaches MAX_PENDING_REQUESTS', async () => {
    // Seed active request + MAX_PENDING_REQUESTS pending drafts
    const session = await sessionStore.loadSession('u1', 's1');
    session.activeRequestId = 'r-existing';
    session.requests.push({
      requestId: 'r-existing', content: 'first', status: 'processing',
    ...
    const { MAX_PENDING_REQUESTS } = await import('../src/agents/session-gate');
    for (let i = 0; i < MAX_PENDING_REQUESTS; i++) {
      session.pendingRequests.push({
        draftId: `d-existing-${i}`, requirement: `existing ${i}`,
        enqueuedAt: new Date().toISOString(), hasImage: false,
      });
    }
    await sessionStore.saveSession('u1', 's1', session);
    await new Promise(r => setTimeout(r, 150));

    const body = JSON.stringify({ requirement: 'overflow', userId: 'u1', sessionId: 's1' });
    const { status, rawBody } = await new Promise<{ status: number; rawBody: string }>((resolve, reject) => {
      const req = http.request(`${url}/tasks/stream`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) },
      }, (res) => {
        let buf = '';
        res.on('data', (c) => buf += c.toString());
        res.on('end', () => resolve({ status: res.statusCode ?? 0, rawBody: buf }));
        res.on('error', reject);
      });
      req.on('error', reject);
      req.write(body);
    ...
    const parsed = JSON.parse(rawBody);
    expect(parsed.code).toBe('QUEUE_FULL');
    expect(parsed.pendingCount).toBe(MAX_PENDING_REQUESTS);
  });
